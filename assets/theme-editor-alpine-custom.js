document.addEventListener('alpine:init', () => {
  ...
  Alpine.data('xVariantSelect', (
    ...
    _setEstimateDelivery(html) {
      ...
      const estimateDeliveryCart = document.getElementsByClassName(`cart-edt-${sectionId}`);
      const estimateDeliveryCartUpdate = html.getElementsByClassName(`cart-edt-${sectionId}`);
    ...
    _handleUpdateProductInfo(html) {
      ...
      const selectors = ['x-block-product-score-', 'block-button-', 'product-description-block-', 'block-inventory-', 'block-available-quantity-', 'quantity-selector-', 'volume-', 'sku-', 'x-badges-', 'preorder-', 'cart-edt-'];
      for (let selector of selectors) {
  ...
  Alpine.data('xStickyATC', (sectionId, is_combined) => ({
    ...
    renderProductPrice(html) {
      const destinations = document.getElementsByClassName(`price-sticky-${sectionId}`);
    ...
    Alpine.data('xBuyOwnBundle', (
      ...
      updateProductQty(el, productId, hasVariant, qty) {
        ...
        if (productsBundle[index]) {
          ...
          this.productsBundle = productsBundle;
          this.updateProgressBar();
          this.updateBundleContent(productsBundle, blockId);
        }
      },
      minusProductQty(el, productId, hasVariant, qty) {
        let productsBundle = JSON.parse(JSON.stringify(this.productsBundle));
